...
class VenvManager:
    ...
    def remove_temp_venv(svc):
        if os.path.exists(svc.config.app_server.venv_base_dir):
            files = os.listdir(svc.config.app_server.venv_base_dir)
            for file_name in files:
                if file_name.startswith("."):
                    try:
                        # os.remove 无法删除.文件
                        if sys.platform == "win32":
                            os.system("rd /s/q {}".format(os.path.join(svc.config.app_server.venv_base_dir, file_name)))
                        else:
                            os.system("rm -rf {}".format(os.path.join(svc.config.app_server.venv_base_dir, file_name)))
                    except Exception as e:
                        pass
                if not os.path.exists(os.path.join(svc.config.app_server.venv_base_dir, file_name, "venv")):
                    try:
                        # os.remove 无法删除.文件
                        if sys.platform == "win32":
                            os.system("rd /s/q {}".format(os.path.join(svc.config.app_server.venv_base_dir, file_name)))
